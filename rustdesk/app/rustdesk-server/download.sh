...
rm -rf amd64
# Use user instead of root to avoid permission issues
sudo -E -u user wget https://github.com/rustdesk/rustdesk-server/releases/latest/download/rustdesk-server-linux-amd64.zip
sudo -E -u user unzip rustdesk-server-linux-amd64.zip

# Create the application directory if it doesn't exist
mkdir -p /app/rustdesk-server

# Copy files to the application directory
sudo -E -u user cp -r amd64/* /app/rustdesk-server/
