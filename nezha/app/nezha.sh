...
install() {
    ...
    # 在自动化模式下，设置默认值
    if [ "$AUTO_INSTALL" = true ]; then
        IS_DOCKER_NEZHA=${NEZHA_DOCKER_INSTALL:-true}
    else
        select_version
    fi
...
modify_config() {
    ...
    # 在自动化模式下，使用环境变量或默认值
    if [ "$AUTO_INSTALL" = true ]; then
        nz_site_title="${NEZHA_SITE_TITLE:-Nezha Dashboard}"
        nz_port="${NEZHA_PORT:-8008}"
        nz_hostport="${NEZHA_AGENT_HOSTPORT:-}"
        nz_tls="${NEZHA_TLS:-false}"
        nz_lang="${NEZHA_LANGUAGE:-zh_CN}"
    else
        printf "请输入站点标题: "
