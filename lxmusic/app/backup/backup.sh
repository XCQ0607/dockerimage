...
s3cmd -c /home/user/s3cfg ls s3://$R2_BUCKET_NAME/ | grep "${BACKUP_NAME}_backup_" | sort -r | awk '{print $4}' | tail -n +$((KEEP_BACKUPS + 1)) | while read -r OLD_BACKUP; do
