...
const handleListAction = async (userName, { action, data }) => {
...
const handler = {
    async onListSyncAction(socket, action) {
        ...
        socket.broadcast((client) => {
            if (client.keyInfo.clientId == currentId || !client.moduleReadys?.list || client.userInfo.name != currentUserName)
                return;
